...
function isPCorLaptop() {
	...
	return (
		/windows nt|macintosh/.test(userAgent) &&
...
function updateRenderer() {
	...
	const containerWidth = modal.classList.contains("show")
...
document.addEventListener("DOMContentLoaded", function () {
	...
	const designOptionsTrigger = document.getElementById(
		"design-options-trigger"
	);
	const designOptionsDropdown = document.getElementById(
		"design-options-dropdown"
	);
	const floorplanOptionsTrigger = document.getElementById(
		"floorplan-options-trigger"
	);
	const floorplanOptionsDropdown = document.getElementById(
		"floorplan-options-dropdown"
	);
	...
	allTriggers.forEach((trigger) => {
		trigger.addEventListener("click", function () {
			const currentDropdown =
				this === designOptionsTrigger
					? designOptionsDropdown
					: floorplanOptionsDropdown;
	...
	function closeOtherDropdowns(currentDropdown) {
		allDropdowns.forEach(function (dropdown, i) {
			if (dropdown !== currentDropdown && dropdown.style.display === "block") {
				...
				if (
					document
						.getElementById("design-options-trigger")
			...
			setTimeout(() => {
				...
				allDropdowns.forEach(function (drop, i) {
					if (drop != dropdown && drop != allDropdowns.at(0)) {
						dropdownContainers[
							i
						].style.transform = `translateY(${dropdown.offsetHeight}px)`;
					}
				});
	...
	class UserPosition {
		...
		determineDirection(rotationY, clickCount) {
			...
			console.log(
				`Normalized Rotation Y: ${normalizedY} radians, ${angleDeg} degrees`
			);
			let direction = null;
		...
		move(direction, clickCount) {
			...
			for (let i = 0; i < moveCount; i++) {
				...
				if (
					tempRow >= 0 &&
					tempRow < this.matrix.length &&
					tempCol >= 0 &&
					tempCol < this.matrix[0].length
				) {
					newRow = tempRow;
					newCol = tempCol;
				} else {
					console.log(
						`Move blocked at step ${i + 1}: target position out of bounds.`
					);
					break;
				}
			}
			...
			console.log(
				`Attempting to move from [${this.row}, ${this.col}] to [${newRow}, ${newCol}]`
			);
		...
		updateSphereTexture(newRow, newCol, guide) {
			...
			textureLoader.load(
				...
				(texture) => {
					...
					if (
						this.currentTexture &&
						this.currentTexture !== sphere.material.map
					) {
	...
	const panoramaMatrix = [
		[
			["panoramas/1_3.webp", "panoramas/2_3.webp"],
			["panoramas/1_2.webp", "panoramas/2_2.webp"],
			["panoramas/1_1.webp", "panoramas/2_1.webp"],
		],
		[
			["panoramas/1_6.webp", "panoramas/2_6.webp"],
			["panoramas/1_5.webp", "panoramas/2_5.webp"],
			0,
		],
	];
	...
	const steps = [
		{
			...
			title: "Welcome!",
			content:
				"Welcome to the Welcome Home platform! This tour will guide you through some of the basic functions of the platform. Click the arrow to get started!",
			placement: "bottom",
		},
		{
			selector: "#info-box",
			...
			title: "Movement",
			content:
				"To move around, double click anywhere. To look around, click and drag the screen.",
			placement: "bottom",
		},
		{
			...
			title: "Menu",
			content:
				"The menu is used to control various parts of the platform as well. Click the menu icon to open the menu.",
			placement: "left",
		},
		{
			...
			title: "Design Center",
			content:
				"The Design Center allows you to view different designs. Click the icon to open the Design Center panel.",
			placement: "bottom",
		},
		{
			...
			step: 5,
			title: "Design Center",
			content: "Click the option to change the current design.",
			placement: "bottom",
		},
		{
			...
			title: "Floor Plans",
			content:
				"Floor Plans allow you to view different floor plans. Click the icon to open the Floor Plans panel.",
			placement: "bottom",
		},
		{
			...
			title: "Floor Plans",
			content:
				"Click the option to change the current floor plan and complete the tour!",
			placement: "bottom",
		},
	];
	...
	var tourguide = new Tourguide({
		...
		onStep: function (currentstep, type) {
			if (currentstep.index == 3) {
				currentstep.container.nodes[0].childNodes[1].children[0].childNodes[2].children[1].classList.toggle(
					"display-as-none"
				);
				...
				currentstep.highlight.nodes[0].addEventListener("click", function () {
					currentDesignStyle = 2;
					userPosition.updateSphereTexture(
						userPosition.row,
						userPosition.col,
						tourguide
					);
...
moaButton.addEventListener("click", function () {
	...
	if (isPCorLaptop()) {
		alert(
			"Motion and orientation features are not supported on PC or laptop devices."
		);
		return;
	}
...
const orientation_controls = new DeviceOrientationControls(camera);
